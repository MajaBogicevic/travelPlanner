...
export default function RegisterPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setApiError(err.response?.data?.message || 'Greska pri registraciji');
        } finally {
            setSubmitting(false);
        }
    };
            ...
            <div style={s.formSide}>
                <div style={s.formBox}>
                    ...
                    <div style={s.formHeader}>
                        ...
                        <p style={s.formSub}>
                            {shareToken ? 'Registrujte se da prihvatite deljeni plan putovanja' : 'Registrujte se i pocnite sa planiranjem'}
                        </p>
                    </div>
                    {shareToken && (
                        <div style={s.shareNotice}>
                            Neko vam je podelio plan putovanja. Nakon registracije, plan ce se pojaviti u sekciji Deljena putovanja.
                        </div>
                    )}
                    <form onSubmit={handleSubmit} autoComplete="off" style={s.form}>
                        <div style={s.field}>
                            <label style={s.label}>Ime i prezime</label>
                            <input style={s.input} name="name" value={form.name} onChange={handleChange} placeholder="Vase ime" />
                        ...
                        <div style={s.field}>
                            ...
                            <input style={s.input} name="password" type="password" value={form.password} onChange={handleChange} placeholder="Minimum 6 karaktera" />
                            {errors.password && <span style={s.fieldError}>{errors.password}</span>}
                        </div>
                        <div style={s.field}>
                            <label style={s.label}>Potvrdi lozinku</label>
                            <input style={s.input} name="confirmPassword" type="password" value={form.confirmPassword} onChange={handleChange} placeholder="Ponovite lozinku" />
                    ...
                    <p style={s.loginLink}>
                        Vec imate nalog?{' '}
                        <Link to={shareToken ? `/login?shareToken=${shareToken}` : '/login'} style={s.link}>
                            Prijavite se
                        </Link>
                    </p>
                </div>
...
const s = {
    ...
    logoWrap: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px', marginBottom: '32px' },
    ...
    fieldError: { fontSize: '12px', color: 'var(--red)' },
